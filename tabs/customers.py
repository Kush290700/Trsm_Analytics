...
def render(df: pd.DataFrame):
    ...
    with st.expander("📊 New / Active / Churn", expanded=False):
        ...
        for prev, curr in zip(months, months[1:]):
            ...
            churn.append({"Month": curr,
                          "ChurnRate": 100 * (1 - len(p & c) / len(p)) if p else None})
        churn_df = pd.DataFrame(churn)
        col1, col2 = st.columns(2)
        col1.plotly_chart(
            px.bar(summary_mon, x="Month", y=["New","Active"],
                   title="New vs Active Customers"), use_container_width=True)
        col2.plotly_chart(
            px.line(churn_df, x="Month", y="ChurnRate",
                    title="Monthly Churn Rate (%)"), use_container_width=True)
    st.markdown("---")

    # CLV & Inter-purchase Interval
    with st.expander("💰 CLV & Inter-purchase", expanded=False):
        clv = dfc.groupby("CustomerName")["Revenue"]
                  .sum().reset_index(name="CLV")
        diffs = compute_interpurchase(dfc)
        c1, c2 = st.columns(2)
        c1.plotly_chart(
            px.histogram(clv, x="CLV", nbins=30, marginal="box",
                         title="Customer Lifetime Value"), use_container_width=True)
        if not diffs.empty:
            c2.plotly_chart(
                px.histogram(diffs, x=diffs.name, nbins=30, marginal="violin",
                             title="Inter-purchase Interval (days)"), use_container_width=True)
    st.markdown("---")

    # RFM Segmentation & Clusters
